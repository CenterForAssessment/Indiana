...
for (al.iter in prior.achievement.levels) {
    ...
    for (i.iter in 1:3) {
        slot.data[YEAR=="2016" & ACHIEVEMENT_LEVEL_PRIOR==al.iter & SGP >= target.ranges.4[[al.name]][[i.iter]][1] & SGP <= target.ranges.4[[al.name]][[i.iter]][2], POINTS:=target.ranges.4[[al.name]][[i.iter]][3]]
    }
}

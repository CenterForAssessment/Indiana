...
Indiana_SGP_LONG_Data_2016_2017 <- Indiana_SGP_LONG_Data[SCHOOL_YEAR %in% c("2016", "2017") & !is.na(SCALE_SCORE)]
Indiana_SGP_LONG_Data_2016_2017[,ACHIEVEMENT_LEVEL:=NULL]
Indiana_SGP_LONG_Data_2016_2017[,ACHIEVEMENT_LEVEL_PRIOR:=NULL]
...
Indiana_SGP <- prepareSGP(Indiana_SGP_LONG_Data_2016_2017)
...
my.tmp <- Indiana_SGP@Data[YEAR=="2017" & !is.na(ACHIEVEMENT_LEVEL_PRIOR),
    list(
...
for (al.iter in prior.achievement.levels) {
    ...
    for (i.iter in 1:3) {
        slot.data[YEAR=="2017" & ACHIEVEMENT_LEVEL_PRIOR==al.iter & SGP >= my.target.ranges[[al.name]][[i.iter]][1] & SGP <= my.target.ranges[[al.name]][[i.iter]][2], POINTS:=my.target.ranges[[al.name]][[i.iter]][3]]
    }
}

...
Indiana_SGP_LONG_Data_2017_2018 <- Indiana_SGP_LONG_Data[SCHOOL_YEAR %in% c("2017", "2018") & !is.na(SCALE_SCORE)]
Indiana_SGP_LONG_Data_2017_2018[,ACHIEVEMENT_LEVEL:=NULL]
Indiana_SGP_LONG_Data_2017_2018[,ACHIEVEMENT_LEVEL_PRIOR:=NULL]
...
Indiana_SGP <- prepareSGP(Indiana_SGP_LONG_Data_2017_2018)
...
my.tmp <- Indiana_SGP@Data[YEAR=="2018" & !is.na(ACHIEVEMENT_LEVEL_PRIOR),
    list(
...
for (al.iter in prior.achievement.levels) {
    ...
    for (i.iter in 1:3) {
        slot.data[YEAR=="2018" & ACHIEVEMENT_LEVEL_PRIOR==al.iter & SGP >= my.target.ranges[[al.name]][[i.iter]][1] & SGP <= my.target.ranges[[al.name]][[i.iter]][2], POINTS:=my.target.ranges[[al.name]][[i.iter]][3]]
    }
}

aggregate.school.data <- slot.data[YEAR=="2018", list(MEDIAN_SGP=median(SGP, na.rm=TRUE), MEAN_POINTS=mean(POINTS, na.rm=TRUE), MEAN_PRIOR_ACHIEVEMENT=mean(SCALE_SCORE_PRIOR_STANDARDIZED, na.rm=TRUE), COUNT=.N), keyby=list(SCHOOL_NUMBER)]
aggregate.school.content_area.data <- slot.data[YEAR=="2018", list(MEDIAN_SGP=median(SGP, na.rm=TRUE), MEAN_POINTS=mean(POINTS, na.rm=TRUE), MEAN_PRIOR_ACHIEVEMENT=mean(SCALE_SCORE_PRIOR_STANDARDIZED, na.rm=TRUE), COUNT=.N), keyby=list(SCHOOL_NUMBER, CONTENT_AREA)]
aggregate.school.content_area.data.g10 <- slot.data[YEAR=="2018" & GRADE=="10", list(MEDIAN_SGP=median(SGP, na.rm=TRUE), MEAN_POINTS=mean(POINTS, na.rm=TRUE), MEAN_PRIOR_ACHIEVEMENT=mean(SCALE_SCORE_PRIOR_STANDARDIZED, na.rm=TRUE), COUNT=.N), keyby=list(SCHOOL_NUMBER, CONTENT_AREA)]
